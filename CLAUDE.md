...
## Layout
- `src/lib/` — pure, unit-tested logic (records, filter, status, enrich, avatar) + boundaries (api, idb, proxy).
- `src/hooks/` — data load (`useIptvData`), playback (`useHlsPlayer`), proxy ping, persistence, media-query, toast, shortcuts.
